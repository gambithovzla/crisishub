"use client";

import { useState } from "react";
import { useTranslations } from "next-intl";
import { toast } from "sonner";
import { Check, Share2 } from "lucide-react";

import { Button } from "@/components/ui/button";

type Props = {
  /** URL absoluta a compartir (enlace limpio). */
  url: string;
  /** Título corto (para Web Share API). */
  title: string;
  /** Texto que precede al enlace. */
  text: string;
  className?: string;
};

/** Icono de WhatsApp (lucide no trae el de marca). */
function WhatsAppIcon({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="currentColor"
      aria-hidden
      className={className}
    >
      <path d="M.057 24l1.687-6.163a11.867 11.867 0 0 1-1.587-5.945C.16 5.335 5.495 0 12.05 0a11.82 11.82 0 0 1 8.413 3.488 11.82 11.82 0 0 1 3.48 8.414c-.003 6.557-5.338 11.892-11.893 11.892a11.9 11.9 0 0 1-5.688-1.448L.057 24zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884a9.86 9.86 0 0 0 1.512 5.26l-.999 3.648 3.985-1.607zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z" />
    </svg>
  );
}

export function ShareButton({ url, title, text, className }: Props) {
  const t = useTranslations("missing");
  const [copied, setCopied] = useState(false);

  const shareText = `${text} ${url}`;

  async function nativeShare() {
    if (typeof navigator !== "undefined" && navigator.share) {
      try {
        await navigator.share({ title, text, url });
        return true;
      } catch {
        // El usuario canceló o no se pudo; seguimos con el respaldo.
      }
    }
    return false;
  }

  async function copyLink() {
    try {
      await navigator.clipboard.writeText(url);
      setCopied(true);
      toast.success(t("shareCopied"));
      setTimeout(() => setCopied(false), 2000);
    } catch {
      // Sin portapapeles: no hacemos nada (el botón de WhatsApp ya sirve).
    }
  }

  async function onShare() {
    const shared = await nativeShare();
    if (!shared) await copyLink();
  }

  const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(shareText)}`;

  return (
    <div className={`flex gap-2 ${className ?? ""}`}>
      <Button
        type="button"
        variant="outline"
        className="flex-1"
        onClick={onShare}
      >
        {copied ? <Check className="size-4" /> : <Share2 className="size-4" />}
        {t("share")}
      </Button>
      <Button
        variant="outline"
        className="flex-1 border-[#25D366]/40 text-[#128C7E] hover:bg-[#25D366]/10 dark:text-[#25D366]"
        render={
          <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" />
        }
      >
        <WhatsAppIcon className="size-4" />
        WhatsApp
      </Button>
    </div>
  );
}
